...
export default function ReportDetailPage({ params }: { params: { id: string } }) {
  const query = useQuery({ queryKey: ["production-report", params.id], queryFn: async () => { const response = await fetch(`/api/production-reports/${params.id}`); const result = await response.json(); if (!response.ok) throw new Error(result.message || "Gagal memuat detail laporan"); return result.data as Report; } });
