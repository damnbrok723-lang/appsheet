    ...
    <div className="space-y-6">
      ...
      {query.isLoading ? <div className="space-y-4">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-20" />)}</div> : query.data && query.data.length > 0 ? (
        <div className="space-y-2">{query.data.map((notification) => (
          <Card key={notification.id} className={`p-4 ${!notification.readAt ? "bg-accent/30" : ""}`}><button type="button" onClick={() => !notification.readAt && markRead.mutate(notification.id)} className="flex w-full items-start gap-3 text-left"><div className="flex-1"><p className="font-medium">{notification.title}</p><p className="text-sm text-muted-foreground">{notification.message}</p><p className="text-xs text-muted-foreground mt-1">{new Date(notification.createdAt).toLocaleString("id-ID")}</p></div>{!notification.readAt && <div className="h-2 w-2 rounded-full bg-primary shrink-0 mt-2" />}</button></Card>
        ))}</div>
      ) : (
